...
- service定義
- SSH向けのような接続頻度制限
- 静的Ethernet forwarding向けの明示的なnftables flowtableヒント
...
通常のruleset生成では、最初の実装としてかなり保守的にしています。
flowtable device listに追加するのは、`eth0`, `eth1` のような実在する
exact Ethernet interfaceだけです。`ppp+`, `wg+`, `br+`, `wlan0`, `wwan0`
のようなprefix表現や動的interfaceは、静的generatorでは追加しません。

これは、設定上は存在していても実機上にまだ存在しないinterfaceによって、
`nft -c` が失敗することを避けるためです。動的interfaceの同期は、将来の
`flowtable-sync` コマンドで扱う想定です。
...
table inet awall_nft
  flowtable ft_forward        # 静的Ethernet flowtable deviceがある場合に出力
  chain input
  chain flowtable_forward
  chain forward
  chain output
...
flowtable ruleを差し込むための安定した場所として生成します。将来の同期処理では、
ruleset全体ではなくこのchainだけを置き換えられます。
...
- SNAT全機能
- ipset/address-set設定
- PPP/WireGuardなど実行時に作られるinterface向けの動的flowtable同期
- `nft -c` 以上のrollback-safe apply

## 将来の拡張候補

- nftables setによるaddress-set/ipset相当の対応
- PPPやWireGuardなどの動的interface向け `flowtable-sync`
- より詳細なvalidate
